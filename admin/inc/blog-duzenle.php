<?php
  ...
  if(is_numeric($_GET["id"])){
    $yazi=$db->select("blog")->where("id",$_GET["id"])->run(true);
    if(empty($yazi)) exit(header("location:".aurl."/index.php?s=blog"));
	...
	if(isset($_POST) and post("btn_kaydet"))
	{
      ...
      if(!empty($_FILES['kapakimg']["name"]))
      {
  			...
  			if ($image->uploaded)
  			{
  				...
  				$uploadyolu = realpath("..").'/uploads/blog/';
      ...
      if(!empty($_FILES['icerikimg']["name"]))
      {
			...
			{
				$ekle = $db->update('blog')->where("id",$yazi["id"])
							->set(array(
								"selflink" => permalink(post("baslik"))."-".mt_rand(10000,99999),
								"katid" => post("kategori"),
								"baslik" => post("baslik"),
								"icerik" => strip_tags(post("icerik",2),$kabulhtmltags),
								"kapak_img" => !empty($kapakimgname) ? "blog/".$kapakimgname : $eski_kapakimgname,
								"icerik_img" => !empty($icerikimgname) ? "blog/".$icerikimgname : $eski_icerikimgname,
				...
				if($ekle)
				{
					...
					header("refresh:2; url=".aurl."/index.php?s=blog");
				}
												...
												<div class="col-md-12">
													...
													<section class="panel">
														<header class="panel-heading">
															<h2 class="panel-title">Blog Yazısı Düzenle</h2>
														</header>
																		...
																		<div class="row">
																			<div class="col-md-8">
																				<select class="form-control" name="kategori">
																					...
																					<?php
																						$katcek=$db->select("blog_kategori")->where("durum",1)->run();
