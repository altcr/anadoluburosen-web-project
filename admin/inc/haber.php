<?php
  if(!empty(get("id"))){
    if(get("durum")=="1"){
      $guncelle=$db->update("haber")->where("id",get("id"))->set(array(
                'durum'=>'0'
              ));
    }
    else{
  ...
  if(!empty(get("sil"))){
    $kontrol=$db->select("haber")->where("id",get("sil"))->run(true);
    if($kontrol){
      unlink(realpath("..").'/uploads/'.$kontrol["kapak_img"]);
      unlink(realpath("..").'/uploads/'.$kontrol["icerik_img"]);
      $sil=$db->delete("haber")->where("id",get("sil"))->done();
...
<div class="row">
  <div class="col-lg-12">
    <!-- start: page -->
          <div class="row">
          <?php
            $yazilar=$db->select("haber")->from("id,durum,kapak_img,icerik,baslik")->orderby("tarih","Desc")->run();
              ...
              echo '<div class="col-md-4">
                    <section class="panel">
                      ...
                      <div class="panel-footer panel-footer-btn-group">
                        <div class="">
                          <a href='.aurl.'/index.php?s=haber&sil='.$val["id"].' class="btn btn-'.$renk.'" onclick="return confirm(\'Yazıyı silmek istiyor musunuz?\');"><i class="fa fa-trash-o mr-xs"></i> Sil</a>
                          <a href='.aurl.'/index.php?s=haber-duzenle&id='.$val["id"].' class="btn btn-'.$renk.'"><i class="fa fa-pencil mr-xs"></i> Düzenle</a>
                          <a href='.aurl.'/index.php?s=haber&durum='.$val["durum"].'&id='.$val["id"].' class="btn btn-'.$renk.'"><i class="fa fa-'.$durum_icon.' mr-xs"></i> '.$durum_yazi.'</a>
                        </div>
                      </div>
                    </section>
                  </div>';
