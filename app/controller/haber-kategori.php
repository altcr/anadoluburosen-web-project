<?php
	$kategori=$db->select("haber_kategori")->where("durum",1)->where("selflink",url(1))->run(true);
	if($kategori){
		...
		$toplamblog = count($db->select("haber")->where("durum",1)->where("katid",$kategori["id"])->run());
		if($toplamblog)
		{
			...
			}else exit(header("Location:".url."/haber"));


			$blog = $db->select("haber")->from("haber.*, haber_kategori.selflink as haberselflink, haber_kategori.ad as haberad")->join("haber_kategori","%s.id=%s.katid and haber_kategori.durum=1","inner")->where("durum",1)->where("katid",$kategori["id"])->orderby("tarih","Desc")->limit($gblog,$blogyazilimit)->run();
