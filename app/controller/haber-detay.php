<?php
	$blog=$db->select("haber")->from("haber.*,haber_kategori.id as katid,haber_kategori.ad as katadi,haber_kategori.selflink as katselflink")->join("haber_kategori","%s.id=%s.katid","inner")->where("durum",1)->where("selflink",url(1))->run(true);
	if($blog){
		$konuetiketler = implode(", ",array_map(function($item){
					return '<a href="'.url.'/haber-etiket/'.permalink(trim($item)).'" title="'.trim($item).'">'.trim($item).'</a>';
		...
		$blogitemurl = url."/haber-detay/".url(1);
	...
	{
		$_sayfatitle = "Haber Bulunamadı";
		$_sayfakeywords ="Hata Sayfası";
		$_sayfadescription = "Böyle bir haber mevcut değil.";
		$viewdegistir="404";
	}
