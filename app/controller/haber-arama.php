<?php
		...
		{
			exit(header("Location:".url."/haber-arama/".urlencode(permalink(post("s")))));
		}

		if(!url(1)) exit(header("Location:".url."/haberler"));

		$sayfanumara = "";
		$toplamblog = count($db->select("haber")->from("haber.id")->join("haber_kategori","%s.id=%s.katid and haber_kategori.durum=1","inner")->where("durum",1)->where("baslik","%".url(1)."%","like")->or_where("icerik","%".url(1)."%","like")->or_where("selflink","%".url(1)."%","like")->or_where("description","%".url(1)."%","like")->or_where("keylink",url(1).",%","like")->or_where("keylink","%,".url(1),"like")->or_where("keylink","%,".url(1).",%","like")->run());
		if($toplamblog)
		{
			...
			if($gecerlis <= $toplams)
			{
				...
				{
					$saynumadet = 10;
					if($gecerlis > $saynumadet/2+1 and $gecerlis < $toplams and $toplams > 1) $sayfanumara .= '<li><a href="'.url.'/haber-arama/'.url(1).'/sayfa/1">«</a></li>';
					for($i = $gecerlis-$saynumadet/2; $i< $gecerlis+$saynumadet/2; $i++)
					{
						if($toplams < $i or $i <=0) continue;
						$sayfanumara .= '<li'.($gecerlis == $i ? ' class="page-item active"' : ' class="page-item"' ).'><a class="page-link" href="'.url.'/haber-arama/'.url(1).'/sayfa/'.$i.'">'.$i.'</a></li>';
					}
					if($toplams - ($saynumadet/2-1) > $gecerlis and $gecerlis < $toplams and $toplams > 1) $sayfanumara .= '<li><a href="'.url.'/haber-arama/'.url(1).'/sayfa/'.$toplams.'">»</a></li>';
				}
			}else exit(header("Location:".url."/haberler"));



			$blog = $db->select("haber")->from("haber.*, haber_kategori.selflink as haberselflink, haber_kategori.ad as haberad")->join("haber_kategori","%s.id=%s.katid and haber_kategori.durum=1","inner")->where("durum",1)->where("baslik","%".url(1)."%","like")->or_where("icerik","%".url(1)."%","like")->or_where("selflink","%".url(1)."%","like")->or_where("description","%".url(1)."%","like")->or_where("keylink",url(1).",%","like")->or_where("keylink","%,".url(1),"like")->or_where("keylink","%,".url(1).",%","like")->limit($gblog,$blogyazilimit)->run();
