<?php
	$kategori=$db->select("blog_kategori")->where("durum",1)->where("selflink",url(1))->run(true);
	if($kategori){
		$_sayfatitle = $kategori["ad"];
		$_sayfakeywords =$kategori["keywords"];
		$_sayfadescription = $kategori["description"];
		$kategoriadi = strto($kategori["ad"]." Kategorisi","b");
		$sayfanumara = "";
		$blogslider = '';

		$toplamblog = count($db->select("blog")->where("durum",1)->where("katid",$kategori["id"])->run());
		if($toplamblog)
		{
			...
			if($gecerlis <= $toplams)
			{
				...
				{
					$saynumadet = 10;
					if($gecerlis > $saynumadet/2+1 and $gecerlis < $toplams and $toplams > 1) $sayfanumara .= '<li><a href="'.url.'/blog-kategori/'.url(1).'/sayfa/1">«</a></li>';
					for($i = $gecerlis-$saynumadet/2; $i< $gecerlis+$saynumadet/2; $i++)
					{
						if($toplams < $i or $i <=0) continue;
						$sayfanumara .= '<li'.($gecerlis == $i ? ' class="page-item active"' : ' class="page-item"' ).'><a class="page-link" href="'.url.'/blog-kategori/'.url(1).'/sayfa/'.$i.'">'.$i.'</a></li>';
					}
					if($toplams - ($saynumadet/2-1) > $gecerlis and $gecerlis < $toplams and $toplams > 1) $sayfanumara .= '<li><a href="'.url.'/blog-kategori/'.url(1).'/sayfa/'.$toplams.'">»</a></li>';
				}
			}else exit(header("Location:".url."/blog"));



			$blog = $db->select("blog")->where("durum",1)->where("katid",$kategori["id"])->limit($gblog,$blogyazilimit)->run();
			...
			$slider = $db->select("blog")->from("blog.*, blog_kategori.selflink as blogselflink, blog_kategori.ad as blogad")->join("blog_kategori","%s.id=%s.katid","inner")->where("durum",1)->where("slider",1)->orderby("tarih","desc")->limit(5)->run();
			if($slider)
			{
				$say = 1;
										...
										<span class="thumb-info thumb-info-centered-info thumb-info-no-borders mb-md">
											<span class="thumb-info-wrapper">
												...
												<span class="thumb-info-title">
													...
													<span class="label label-'.$renk[($kategori["id"] % 10)].'" style="">'.strto($val["blogad"],"b").'</span>
												</span>
												<span class="thumb-info-action">
													<span class="thumb-info-action-icon"><i class="fa fa-link"></i></span>
												</span>
		...
		$_sayfatitle = "Kategori Bulunamadı";
		$_sayfakeywords ="Hata Sayfası";
		$_sayfadescription = "Böyle bir kategori mevcut değil.";
		$viewdegistir="404";
	}
