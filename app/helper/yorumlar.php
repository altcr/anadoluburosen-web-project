<?php
	function yorumlar($a, $b, $c = -1, $d = false)
	{
		global $db;
		if($d === true)
			return count($db->select("yorumlar")->where("tip",$a)->where("itemid",$b)->where("durum",1)->run());
		$yorumlar = $db->select("yorumlar")->where("tip",$a)->where("itemid",$b)->where("altid",$c)->where("durum",1)->run();
		if($yorumlar)
			foreach($yorumlar as $val)
			{
				...
				$altyorumlar = $db->select("yorumlar")->where("tip",$a)->where("itemid",$b)->where("altid",$val["id"])->where("durum",1)->run();
				if($altyorumlar)
				{
