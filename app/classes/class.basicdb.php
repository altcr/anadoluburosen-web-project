...
class BasicDB extends \PDO
{
    ...
    public function where($column, $value = '', $mark = '=', $logical = '&&',$xtable = null)
    {
		if($xtable == null) $xtable = $this->tableName;
        $this->where[] = array(
            $column,
            $value,
            $mark,
        ...
        if (is_array($this->where) && count($this->where) > 0) {
            ...
            foreach ($this->where as $key => $arg) {
                if (strstr($arg[1], 'FIND_IN_SET')) {
                    ...
                    $where[] = ($key > 0 ? $arg[3] : null) .' '.$arg[4]. '.`' . $arg[0] . '` ' . strtoupper($arg[2]) . ' ' . (strstr($arg[2], 'IN') ? '(' : '"') . $arg[1] . (strstr($arg[2], 'IN') ? ')' : '"');
                }
            }
	...
	public function sql($sql,$u = false){
		$this->sql = $sql;
        if ($u){
            return $this->query($sql)->fetch(parent::FETCH_ASSOC);
        }
        else{
            return $this->query($sql)->fetchAll(parent::FETCH_ASSOC);
        }
	}
